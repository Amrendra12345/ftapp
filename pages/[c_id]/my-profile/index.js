...
const My_profile = () => {
	...
	const updateProfile = () => {
		...
		axios.post(`https://cms.fasttrackvisa.com/api/${c_ext}/updateprofile`, data).then((res) => {
			console.log(res);
	...
	return (
		<>
			...
			<header className={'App-header' + ` ${activeClass}`}>
				<Nav ce_name={c_ext} lob_name="home_page" meta_title={'Apply Global eVisa | Tourist &amp; Business eVisa Online Services | Fast Track Visa'} meta_desc={'Apply for e-visa, tourist &amp; business visa globally online Through Fast Track Visa. We ensure fewer chances of rejection, world-class security, 24/7 customer support &amp; a speedy process.'} meta_keyword="" meta_img="/img/logo.png" meta_url={'my-profile'}> </Nav>
			</header>
					...
					<Row>
						<Col sm={12} md={3} lg={3}>
							<ul className='profile_ul'>
								<li><Link href={'/' + c_ext + '/my-profile/my-transactions'}><a><i className='fa fa-list'></i> My Transactions</a></Link></li>
								<li><Link href={'/' + c_ext + '/my-profile/change-password'}><a><i className='fa fa-key'></i> Change Password</a></Link></li>
								<li><Link href={'/' + c_ext + '/my-profile'}><a><i className='fa fa-user'></i> Update Profile</a></Link></li>
							</ul>
						</Col>
						<Col sm={12} md={9} lg={9}>
